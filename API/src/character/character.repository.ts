...
export class CharacterRepository implements Repository<Character>{
    ...
    public update(item: Character): Character | undefined {
        const characterIdx = characters.findIndex((character) => character.id === item.id)   //findIndex devuelve indice del arreglo donde esta ese character, si lo hiciera sin index crearía otro character en memoria 
